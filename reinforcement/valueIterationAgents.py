...
class ValueIterationAgent(ValueEstimationAgent):
    ...
    def __init__(self, mdp, discount = 0.9, iterations = 100):
        ...
        for _ in range(iterations):
          ...
          for s in self.mdp.getStates():
            ...
            new_values[cur_state] = self.getQValue(cur_state,action)
    ...
    def computeQValueFromValues(self, state, action):
        ...
        for x,y in states_probs:
          R = self.mdp.getReward(state,action,x)
          q += (R + self.discount*self.getValue(x))*y
    ...
    def computeActionFromValues(self, state):
        ...
        for x,y in Qs.items():
          if y == m:
            choose_actions.append(x)
        return choose_actions[random.randint(0,len(choose_actions)-1)]
